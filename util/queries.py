...
# Build connect to Pokemon Stats Database#
...
# Build connect to MAD/RDM Database#
...
async def statboard_mon_today(config, area, use_alt_table=False):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_mon_today.execute(f"select count(id) from pokemon_history WHERE CONVERT_TZ(from_unixtime(first_seen_timestamp),'+00:00','{config['timezone']}') > CURDATE() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_hundos_today(config, area, use_alt_table=False):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_hundos_today.execute(f"select count(id) from pokemon_history where iv = 100 AND CONVERT_TZ(from_unixtime(first_seen_timestamp),'+00:00','{config['timezone']}') > CURDATE() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_iv0_today(config, area, use_alt_table=False):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_iv0_today.execute(f"select count(id) from pokemon_history where iv = 0 AND CONVERT_TZ(from_unixtime(first_seen_timestamp),'+00:00','{config['timezone']}') > CURDATE() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_scanned_today(config, area, use_alt_table=False):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_scanned_today.execute(f"select count(id) from pokemon_history where iv is not NULL AND CONVERT_TZ(from_unixtime(first_seen_timestamp),'+00:00','{config['timezone']}') > CURDATE() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_total_iv_today(config, area, use_alt_table=False):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_total_iv_today.execute(f"select ifnull(sum(iv),0) from pokemon_history where iv is not NULL AND CONVERT_TZ(from_unixtime(first_seen_timestamp),'+00:00','{config['timezone']}') > CURDATE() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_raid_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        await cursor_statboard_raid_active.execute(f"select count(raid.gym_id), ifnull(sum(level = 1), 0), ifnull(sum(level = 2),0), ifnull(sum(level = 3),0), ifnull(sum(level = 4),0), ifnull(sum(level = 5),0), ifnull(sum(level = 6),0), ifnull(sum(level = 7),0) from gym left join raid on gym.gym_id = raid.gym_id where end >= utc_timestamp() and start <= utc_timestamp() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(latitude, longitude))")
    elif config['db_scan_schema'] == "rdm":
        await cursor_statboard_raid_active.execute(f"select count(id), ifnull(sum(raid_level = 1), 0), ifnull(sum(raid_level = 2),0), ifnull(sum(raid_level = 3),0), ifnull(sum(raid_level = 4),0), ifnull(sum(raid_level = 5),0), ifnull(sum(raid_level = 6),0), ifnull(sum(raid_level = 7),0) from gym where raid_battle_timestamp < UNIX_TIMESTAMP() AND raid_end_timestamp >= UNIX_TIMESTAMP() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_egg_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_egg_active.execute(f"select count(id), ifnull(sum(raid_level = 1), 0), ifnull(sum(raid_level = 2),0), ifnull(sum(raid_level = 3),0), ifnull(sum(raid_level = 4),0), ifnull(sum(raid_level = 5),0), ifnull(sum(raid_level = 6),0), ifnull(sum(raid_level = 7),0) from gym WHERE raid_battle_timestamp >= UNIX_TIMESTAMP() and ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_grunt_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_grunt_active.execute(f"SELECT count(id) FROM Get_Grunts WHERE ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
...
async def statboard_leader_active(config, area):
    ...
    if config['db_scan_schema'] == "mad":
        ...
        await cursor_statboard_leader_active.execute(f"SELECT count(id) FROM Get_Grunts WHERE ST_CONTAINS(ST_GEOMFROMTEXT('POLYGON(({area}))'), point(lat, lon))")
